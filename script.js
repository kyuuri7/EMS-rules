...
// 本文を差し替えて文字モーションを再適用する（検索ハイライト時は演出なしで即表示）
function setBodyHtml(html) {
  bodyEl.innerHTML = html;
  enhanceCodeBlocks(bodyEl);
  if (window.EMSFX) {
    EMSFX.apply(bodyEl, { instant: true });
    EMSFX.initOdometers(bodyEl, { instant: true });
  }
}
...
async function loadPage(page, headingId = null) {
  ...
  const swap = () => {
    if (pageTransitionEl) {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          pageTransitionEl.classList.add('is-visible');
          spawnPageBurst();
        });
      });
    }
  ...
  try {
    ...
    setupScrollSpy();
    swap();
    // ページのフェードインが始まってから、ぼやっと順々に表示する演出を開始
    if (window.EMSFX) {
      setTimeout(() => EMSFX.apply(bodyEl), 260);
      EMSFX.initOdometers(bodyEl);
    }
...
// ---------- コードブロックの強化（まるごとコピー ＋ 部分コピーのチップ） ----------
// content/*.md の中で ```コード``` を使うと、
//  ・右上に「コピー」ボタンが自動でつく（ブロックの中身をまるごとコピー）
//  ・さらにその中で {{ }} で囲んだ文字は、その部分だけをワンクリックでコピーできる
//    小さな「チップ」ボタンに変換される（コードブロックの外の普通の文章でも使える）
//    例）「{{T}}」を押した後に「{{/nametagmenu}}」を入力して…
// 「」は普通の日本語の引用符としてあちこちで使われているので、コピー対象の目印には
// 「」ではなくこの {{ }} を使う（{{ }} は本文中でまず使われない記号なので安全）。

// クリップボードへのコピー（対応ブラウザは navigator.clipboard、
// 使えない環境向けに古い方式のフォールバックも用意）
...
// pre要素の「見た目としてのコピー用テキスト」を作る。
// 中の .copy-chip（部分コピー用チップ）はアイコンを外して元の文字だけに戻し、
// まるごとコピー用ボタン自体が紛れ込んでいれば取り除く。
function getPlainCopyText(el) {
  const clone = el.cloneNode(true);
  clone.querySelectorAll('.copy-chip').forEach(chip => {
    chip.replaceWith(chip.dataset.copy ?? chip.textContent);
  });
  clone.querySelectorAll('.code-copy-btn').forEach(btn => btn.remove());
  return clone.textContent;
}

// ボタンの見た目を一瞬「コピーしました」状態にする共通処理
function flashCopied(btn, iconEl, doneIcon) {
  const original = iconEl.textContent;
  btn.classList.add('is-copied');
  iconEl.textContent = doneIcon;
...
// {{ }} の中身を HTML の文字列として一時的に解釈し、
// タグが混ざっていても（例: <br> や、コードブロック内でエスケープされた &lt;br&gt;）
// 必ず「見た目どおりの生の文字」に揃える。
// これにより、```で囲んでいなくても <br> 入りの文字がその場で改行されてしまうことなく、
// 常に文字どおり表示・コピーされるようになる。
function htmlFragmentToLiteralText(fragment) {
  const div = document.createElement('div');
  div.innerHTML = fragment;
  let out = '';
  div.childNodes.forEach(node => {
    if (node.nodeType === Node.TEXT_NODE) out += node.nodeValue;
    else if (node.nodeType === Node.ELEMENT_NODE) out += node.outerHTML;
...
// {{ }} で囲まれた部分をコピーチップのHTMLに置き換える（文字列レベルで処理するため、
// ```コードブロックの中でも外の普通の文章の中でも同じように動く）
function convertCopyChipsInHtml(html) {
  return html.replace(/\{\{([\s\S]*?)\}\}/g, (whole, raw) => {
    const literal = htmlFragmentToLiteralText(raw);
    if (!literal) return whole;
    const safe = escapeHtml(literal);
    return `<button type="button" class="copy-chip" data-copy="${safe}" aria-label="「${safe}」をコピー" title="クリックでコピー"><span class="copy-chip-text">${safe}</span><span class="copy-chip-icon">⧉</span></button>`;
  });
}

function enhanceCodeBlocks(container) {
  // ① {{ }} で囲んだ部分を、ワンクリックでコピーできる小さなチップに変換する。
  //    ```コードブロックの中・普通の文章中のどちらでもOK。HTMLの文字列として
  //    まとめて処理するので、<br> のようなタグが混ざっていても文字どおり扱われる。
  if (container.innerHTML.includes('{{')) {
    container.innerHTML = convertCopyChipsInHtml(container.innerHTML);
  }

  // ② ```コードブロック``` を「ヘッダーバー＋本文」の枠で包み、
  //    ヘッダーバーの右端に「まるごとコピー」ボタンを追加する
  //    （本文の折り返しとボタンが重ならないよう、ボタンは本文の外＝ヘッダーバー側に置く）
  container.querySelectorAll('pre').forEach(pre => {
    if (pre.parentElement?.classList.contains('code-block')) return; // 二重処理防止
    ...
    label.innerHTML = '<span aria-hidden="true">▍</span>コード';

    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'code-copy-btn';
    btn.setAttribute('aria-label', 'コードをまるごとコピー');
    btn.title = 'コードをまるごとコピー';
    const icon = document.createElement('span');
    icon.className = 'copy-icon';
    icon.textContent = '📋';
    const btnLabel = document.createElement('span');
    btnLabel.className = 'copy-label';
    btnLabel.textContent = 'コピー';
    btn.appendChild(icon);
    btn.appendChild(btnLabel);

    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const ok = await copyTextToClipboard(getPlainCopyText(pre));
      if (ok) {
...
// .copy-chip は container.innerHTML の書き換えで生成される（イベントリスナーを
// 個別に付けられない）ため、クリックは bodyEl でまとめて受け取る（イベント委譲）。
// ページを何度差し替えてもこのリスナー自体は1回登録すればよい。
...
function highlightQueryInBody(query) {
  if (!query) { setBodyHtml(currentRawHtml); return; }
  const re = new RegExp(`(${escapeRegExp(query)})`, 'gi');
  const wrapper = document.createElement('div');
...
searchBox.addEventListener('input', () => {
  ...
  if (!q) setBodyHtml(currentRawHtml);
...
// ---------- ヒーローの文字モーション ----------
if (window.EMSFX) EMSFX.apply(document.getElementById('site-hero'));

// ---------- ページ切替時の粒子バースト ----------
// 「切り替わった」瞬間を強調するため、コンテンツ上部あたりに
// 金粉を一瞬パッと散らす。常時舞っている page-particles とは別の演出。
function spawnPageBurst() {
  if (prefersReducedMotion) return;
  const wrap = document.getElementById('page-particles');
  if (!wrap) return;

  const burst = document.createElement('div');
  burst.className = 'page-burst';
  const isMobile = window.matchMedia('(max-width: 640px)').matches;
  const bandTop = isMobile ? 118 : 150;

  const COUNT = 16;
  for (let i = 0; i < COUNT; i++) {
    const p = document.createElement('span');
    p.className = 'page-burst-particle';
    const angle = Math.random() * Math.PI * 2;
    const dist = 26 + Math.random() * 60;
    p.style.setProperty('--dx', `${Math.cos(angle) * dist}px`);
    p.style.setProperty('--dy', `${Math.sin(angle) * dist}px`);
    p.style.left = `${38 + Math.random() * 24}%`;
    p.style.top = `${bandTop + Math.random() * 26}px`;
